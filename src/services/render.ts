...
function layout(title: string, active: string, body: string) {
  const nav = [
    { href: "/", label: "Overview" },
    { href: "/retention-lane", label: "Retention Lane" },
    { href: "/case-risks", label: "Case Risks" },
    { href: "/disposition-posture", label: "Disposition Posture" },
...
<html lang="en">
  ...
  <body>
    ...
    <div class="wrap">
      <div class="topbar">
        <div class="left">Kinetic Gain · Purview Retention Control Plane</div>
        <div class="right">
          <div>synthetic purview retention · ediscovery packets</div>
          <div>azure · microsoft 365 · purview · legal ops</div>
        </div>
      </div>
      <div class="herorow">
        <section class="hero">
          <div class="chiprow">
            <span class="meta-chip">Wave 11 · Cloud Identity &amp; Device Control</span>
            <span class="meta-chip">Azure / Microsoft 365 / Purview proof</span>
            <span class="meta-chip">Synthetic retention + eDiscovery exports</span>
          </div>
          <h1>Purview retention, label disposition, and case posture that stay operator-readable.</h1>
          <p>This control plane turns raw Microsoft Graph Purview exports into a buyer-readable compliance and legal-ops surface: workload coverage, label-quality drift, stale cases, failed holds, and the remediation packet needed before audits, investigations, or disposition reviews break.</p>
          <div class="navrow">
        ...
        <aside class="side">
          <div class="bluf">
            ...
            <p><strong>Enterprise Purview retention and eDiscovery operations for Microsoft 365, legal, and compliance teams.</strong><br />Audit-safe visibility into retention coverage, label disposition, and matter health without tenant-secret exposure.</p>
          </div>
          <div class="corr">
            <div class="lbl">Proof Layer</div>
            <p><strong>Offline analyzer + CLI + dashboard surface.</strong><br />This repo includes a reusable analyzer that reads Microsoft Graph retention and eDiscovery exports and turns them into workload, case, and disposition packets.</p>
          </div>
          <div class="corr">
            <div class="lbl">Why it matters</div>
            <p>Recruiters looking for <strong>Azure / Microsoft 365 / Purview</strong> should see real retention and legal-ops work: workload coverage, custodian holds, label behavior, and matter-readiness controls.</p>
          </div>
        </aside>
      </div>
      ${body}
      <div class="footer">
        <div>m365-retention-case-orchestrator · synthetic sample data only</div>
        <div>routes: / · /retention-lane · /case-risks · /disposition-posture · /verification · /docs</div>
      </div>
    </div>
  </body>
</html>`;
...
export function renderOverview() {
  ...
  return layout(
    ...
    `<section class="section">
        <div class="sh"><h2>Operator Snapshot</h2><div class="note">retention coverage · eDiscovery health</div></div>
        <div class="kpis">
          <div class="kpi cyan"><div class="v">${metrics.retentionPolicies}</div><div class="lbl">retention policies</div><div class="h">Synthetic policy records covering M365 workloads and retention behavior.</div></div>
          <div class="kpi green"><div class="v">${metrics.retentionLabels}</div><div class="lbl">retention labels</div><div class="h">Labels with disposition and record-behavior posture in scope.</div></div>
          <div class="kpi plum"><div class="v">${metrics.cases}</div><div class="lbl">eDiscovery cases</div><div class="h">Active, stale, or error-state matters visible in one compliance surface.</div></div>
          <div class="kpi red"><div class="v">${metrics.uncoveredWorkloads}</div><div class="lbl">uncovered workloads</div><div class="h">Required workloads missing active retention coverage.</div></div>
          <div class="kpi amber"><div class="v">${metrics.pendingOrErrorHolds}</div><div class="lbl">hold issues</div><div class="h">Custodian holds still pending or already failed on open matters.</div></div>
          <div class="kpi red"><div class="v">${metrics.highFindings}</div><div class="lbl">high findings</div><div class="h">High-severity retention and eDiscovery issues needing operator intervention first.</div></div>
        </div>
      </section>
      <section class="section">
        <div class="sh"><h2>Why operators care</h2><div class="note">purview posture · label quality · matter readiness</div></div>
        <div class="stack">
          <div class="src"><div class="src-name">records governance</div><div class="src-tit">Clear workload coverage first</div><p>${metrics.recommendation}</p></div>
          <div class="src"><div class="src-name">case evidence</div><div class="src-tit">Turn exports into operator proof</div><p>Every lane stays tied to owner, policy or case focus, finding severity, and the next concrete remediation move.</p></div>
          <div class="src"><div class="src-name">recruiter signal</div><div class="src-tit">Show real Microsoft admin depth</div><p>This is real Microsoft 365 / Purview / eDiscovery proof, not generic cloud copy.</p></div>
        </div>
      </section>`
  );
}

export function renderRetentionLane() {
  return layout(
    "M365 Retention Case Orchestrator — Retention Lane",
    "/retention-lane",
    `<section class="section">
        <div class="sh"><h2>Retention Lane</h2><div class="note">owner · workload · next action</div></div>
        <table class="ttbl">
          <thead>
            <tr>
              <th>Lane</th>
...
export function renderCaseRisks() {
  return layout(
    "M365 Retention Case Orchestrator — Case Risks",
    "/case-risks",
    `<section class="section">
        <div class="sh"><h2>Case Risks</h2><div class="note">severity · owner · subject</div></div>
        <table class="ttbl">
          <thead>
            <tr>
              <th>Risk</th>
            ...
            ${caseRisks()
              .map(
                (finding) => `<tr>
                    ...
                    <td>${finding.owner}</td>
                    <td>${finding.subjectName ?? finding.subject}</td>
                    <td>${finding.custodian ?? "—"}</td>
...
export function renderDispositionPosture() {
  return layout(
    "M365 Retention Case Orchestrator — Disposition Posture",
    "/disposition-posture",
    `<section class="section">
        <div class="sh"><h2>Disposition Posture</h2><div class="note">packet readiness · blocker · review window</div></div>
        <div class="board">
          ${dispositionPosture()
            .map(
...
export function renderVerification() {
  return layout(
    "M365 Retention Case Orchestrator — Verification",
    "/verification",
    `<section class="section">
        ...
        <div class="stack">
          ${verification()
            .map(
              (item, index) => `<div class="src"><div class="src-name">verification ${index + 1}</div><div class="src-tit">${item}</div><p>This surface is built to stay honest about offline exports, synthetic sample data, and real Microsoft retention and eDiscovery posture.</p></div>`
            )
            .join("")}
        </div>
      </section>`
...
export function renderDocs() {
  return layout(
    "M365 Retention Case Orchestrator — Docs",
    "/docs",
    `<section class="section">
        <div class="sh"><h2>Docs</h2><div class="note">routes · cli · api</div></div>
        <div class="stack">
          <div class="src"><div class="src-name">routes</div><div class="src-tit">Public control surface</div><p><code>/</code>, <code>/retention-lane</code>, <code>/case-risks</code>, <code>/disposition-posture</code>, <code>/verification</code>, <code>/docs</code></p></div>
          <div class="src"><div class="src-name">api</div><div class="src-tit">Structured payloads</div><p><code>/api/dashboard/summary</code>, <code>/api/retention-lane</code>, <code>/api/case-risks</code>, <code>/api/disposition-posture</code>, <code>/api/verification</code>, <code>/api/sample</code></p></div>
          <div class="src"><div class="src-name">cli</div><div class="src-tit">Offline Graph export analysis</div><p><code>npx m365-retention-case fixtures/compliance-clean.json --format summary</code> renders the same analyzer posture the dashboard exposes.</p></div>
        </div>
      </section>`
  );
}
